...
document.addEventListener('mousemove', (e) => {
  ...
  document.addEventListener('contextmenu', (e) => {
    ...
    // Handle link selection
        ...
        // Get include/exclude keywords from current action
        const includeKeywords = currentMatchedAction.includeKeywords ?
            currentMatchedAction.includeKeywords.split(',').map(k => k.trim().toLowerCase()).filter(k => k) : [];

        const excludeKeywords = currentMatchedAction.excludeKeywords ?
            currentMatchedAction.excludeKeywords.split(',').map(k => k.trim().toLowerCase()).filter(k => k) : [];
        ...
        links.forEach(link => {
            ...
            // If the link is in the box, check keywords, add it to selectedLinks and highlight it
            if (isInBox) {
                const url = link.href.toLowerCase();

                // Check exclude keywords first - if any match, skip this link
                ...
                if (includeKeywords.length > 0) {
                    if (includeKeywords.some(keyword => url.includes(keyword))) {
                    ...
                    // No include keywords specified, include all links that weren't excluded
                    selectedLinks.add(link);
                    link.style.backgroundColor = `${currentMatchedAction.boxColor}33`;
                }
            } else {
